...
import { 
  Menu, Search, ShieldCheck, Shield, ShoppingBag, Plus, Sparkles, ChevronDown, Lock, X, Star, Bot, CheckCircle,
  Instagram, Facebook, Twitter, Youtube, MapPin, Phone, Mail, Linkedin, ArrowRight, RefreshCcw, Camera, UploadCloud
} from 'lucide-react';
...
export default function FrontPageStore() {
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
      ...
      try {
          const payload = { 
              ...
              media: reviewMedia, 
              product: 'GLOBAL', 
              visibility: 'pending', // 🌟 ALWAYS PENDING 🌟
  ...
  return (
    <div className="bg-[#FAFAFA] text-[#050505] font-sans selection:bg-[var(--theme-primary)] selection:text-white overflow-x-hidden">
      ...
      {/* 🌟 USER REVIEW MODAL WITH MEDIA UPLOAD 🌟 */}
      <AnimatePresence>
        {isReviewModalOpen && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[300] bg-black/80 flex items-center justify-center p-4 md:p-6 backdrop-blur-sm overflow-y-auto">
            <div className="bg-white p-8 md:p-10 rounded-3xl w-full max-w-lg relative my-8 shadow-2xl">
               <button onClick={() => setIsReviewModalOpen(false)} className="absolute top-6 right-6 bg-gray-100 text-gray-500 rounded-full p-2 hover:text-black transition-colors"><X size={20}/></button>
               
               {reviewStatus === 'success' ? (
                   <div className="text-center py-10">
                       <CheckCircle size={60} className="text-green-500 mx-auto mb-4" />
                       <h3 className="text-2xl font-serif text-black mb-2">Thank You!</h3>
                       <p className="text-gray-500 text-sm font-serif">Your review has been successfully posted.</p>
                   </div>
               ) : (
                   <>
                       ...
                       <div className="space-y-5">
                           <div>
                              <label className="text-xs font-bold text-gray-600 uppercase mb-2 block">Your Name</label>
                              <input value={reviewForm.userName} onChange={e=>setReviewForm({...reviewForm, userName: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-4 rounded-xl text-sm outline-none focus:border-black" placeholder="John Doe"/>
                           </div>
                           ...
                           <div>
                               ...
                               <div className="flex gap-2">
                                   {[1,2,3,4,5].map(star => (
                                       <button key={star} onClick={() => setReviewForm({...reviewForm, rating: star})} className={`transition-all ${reviewForm.rating >= star ? 'text-[#D4AF37]' : 'text-gray-300'}`}>
                           ...
                           <div>
                               <label className="text-xs font-bold text-gray-600 uppercase mb-2 block">Your Review</label>
                               <textarea value={reviewForm.comment} onChange={e=>setReviewForm({...reviewForm, comment: e.target.value})} rows={3} className="w-full bg-gray-50 border border-gray-200 p-4 rounded-xl text-sm outline-none focus:border-black custom-scrollbar" placeholder="I really loved this..."/>
                           </div>
                           
                           {/* MEDIA UPLOAD SECTION */}
                           <div className="border-t border-gray-100 pt-4">
                               <label className="text-xs font-bold text-gray-600 uppercase mb-3 flex items-center gap-2"><Camera size={14}/> Attach Photos / Videos</label>
                               ...
                               <div className="flex flex-wrap gap-3">
                                   {reviewMedia.map((url, idx) => (
                                       <div key={idx} className="relative w-16 h-16 rounded-lg overflow-hidden border border-gray-200">
                                           ...
                                           <button onClick={()=>setReviewMedia(reviewMedia.filter(x => x !== url))} className="absolute top-1 right-1 bg-black/70 text-white rounded-full p-0.5 hover:bg-red-500"><X size={10}/></button>
                                       </div>
                                   ))}
                                   
                                   <div className="relative w-16 h-16 rounded-lg border-2 border-dashed border-gray-300 hover:border-black hover:bg-gray-50 transition-colors flex flex-col items-center justify-center cursor-pointer">
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div initial={{opacity:0, backdropFilter:'blur(0px)'}} animate={{opacity:1, backdropFilter:'blur(20px)'}} exit={{opacity:0, backdropFilter:'blur(0px)'}} className="fixed inset-0 z-[300] bg-black/80 flex flex-col p-10">
            <div className="flex justify-end mb-20"><X size={32} className="text-[#D4AF37] cursor-pointer" onClick={()=>setIsMenuOpen(false)} /></div>
            <nav className="space-y-10 text-center font-serif text-5xl font-black italic tracking-tighter flex flex-col">
                {["Home", "Our Collection", "Our Story", "Checkout"].map(m => (
                    <Link key={m} href={m === "Home" ? "/" : m === "Checkout" ? "/checkout" : `#${m.toLowerCase().replace(' ','')}`} onClick={()=>setIsMenuOpen(false)} className="text-white hover:text-[#D4AF37] transition-colors">
                        {m}
                    </Link>
                ))}
            </nav>
        ...
        <div className="flex items-center justify-between px-6 md:px-16 h-full relative">
          ...
          <div className="flex items-center gap-6 md:gap-8">
            ...
            <div className="relative cursor-pointer group" onClick={() => router.push('/checkout')}>
              ...
              {cart.length > 0 && <span className="absolute -top-2 -right-2 bg-[var(--theme-primary)] text-black w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black">{cart.length}</span>}
            </div>
          </div>
        </div>
      </nav>
      ...
      {latestWatches.length > 0 && (
          <section className="py-24 bg-white border-b border-gray-100 relative overflow-hidden">
             ...
             <div className="w-full overflow-x-auto custom-scrollbar snap-x snap-mandatory scroll-pl-6 md:scroll-pl-12">
                 <div className="flex gap-6 px-6 md:px-12 w-max pb-10">
                     {latestWatches.map((watch: any, i: number) => (
                         <Link href={`/product/${watch.slug || watch._id}`} key={`horiz-${i}`} className="w-[280px] md:w-[350px] shrink-0 snap-start bg-[#FAFAFA] rounded-[30px] p-6 border border-gray-200 group hover:border-[var(--theme-primary)] transition-colors cursor-pointer relative block">
                            ...
                            <div className="h-48 md:h-64 bg-white rounded-2xl mb-6 p-4 flex items-center justify-center relative">
      ...
      <section id="ourcollection" className="py-32 px-6 md:px-12 max-w-[1800px] mx-auto bg-[#FAFAFA] relative overflow-hidden">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-end mb-20 border-b border-gray-200 pb-10 relative z-10">
          ...
          <div className="w-full xl:w-auto mt-10 xl:mt-0 flex flex-col items-end gap-6">
             <div className="flex items-center gap-3 bg-white px-6 py-3 rounded-full border border-gray-200 shadow-sm w-full md:w-80">
             ...
             <div className="flex flex-wrap gap-3 justify-end">
               {categories.map((cat: any, i: number) => (
                 <button key={`cat-${i}`} onClick={() => setActiveCategory(cat)} className={`px-6 py-3 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${activeCategory === cat ? 'bg-[#00150F] text-[var(--theme-primary)]' : 'bg-white border border-gray-200 text-gray-500 hover:border-black hover:text-black'}`}>{cat}</button>
               ))}
             </div>
          </div>
        </div>
          ...
          {filteredWatches.length === 0 ? (
            ...
            <AnimatePresence mode='popLayout'>
              {filteredWatches.map((watch: any, i: number) => (
                <motion.div key={watch._id || `watch-${i}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{opacity:0, scale:0.9}} transition={{ duration: 0.4 }} className="group bg-white p-8 rounded-[40px] border border-gray-100 hover:shadow-2xl hover:border-[var(--theme-primary)]/30 transition-all duration-500 flex flex-col h-full relative">
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-end mt-auto pt-6 border-t border-gray-100">
                      ...
                      <button onClick={(e) => { e.preventDefault(); addToCart(watch); }} className="p-4 bg-black text-white rounded-2xl hover:bg-[var(--theme-primary)] hover:text-black transition-colors shadow-lg"><Plus size={20}/></button>
                    </div>
                  </div>
                </motion.div>
              ))}
      ...
      <section className="py-20 md:py-32 bg-white border-t border-gray-100 hidden md:block">
        <div className="max-w-[1600px] mx-auto flex items-center justify-center gap-6 lg:gap-10 px-10">
          <motion.div style={{ y: y1 }} className="w-1/3 flex flex-col gap-6 lg:gap-10 pt-20">
            <img src={galleryImages[0]} className="w-full h-[300px] lg:h-[400px] object-cover rounded-[30px] lg:rounded-[40px] shadow-lg filter contrast-110" />
            <img src={galleryImages[3]} className="w-full h-[250px] lg:h-[300px] object-cover rounded-[30px] lg:rounded-[40px] shadow-lg filter contrast-110" />
          </motion.div>
          <div className="w-1/3 flex flex-col gap-6 lg:gap-10">
            ...
            <img src={galleryImages[2]} className="w-full h-[400px] lg:h-[500px] object-cover rounded-[30px] lg:rounded-[40px] shadow-lg filter contrast-110" />
          </div>
          <motion.div style={{ y: y2 }} className="w-1/3 flex flex-col gap-6 lg:gap-10 pt-40">
            <img src={galleryImages[4]} className="w-full h-[350px] lg:h-[450px] object-cover rounded-[30px] lg:rounded-[40px] shadow-lg filter contrast-110" />
            <img src={galleryImages[5]} className="w-full h-[250px] lg:h-[350px] object-cover rounded-[30px] lg:rounded-[40px] shadow-lg filter contrast-110" />
          </motion.div>
        </div>
      </section>
      ...
      <section id="reviews" className="py-24 bg-white border-t border-gray-100 overflow-hidden relative">
          ...
          <div className="text-center mb-20 relative z-10 px-6">
             ...
             <button onClick={() => setIsReviewModalOpen(true)} className="mt-10 px-10 py-5 bg-[#050505] text-white rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-[var(--theme-primary)] hover:text-black transition-colors shadow-[0_10px_30px_rgba(0,0,0,0.2)] flex items-center justify-center mx-auto gap-2">
          ...
          {flowingReviews.length > 0 && (
              <div className="flex w-[200%] relative z-10">
                <motion.div animate={{ x: ["0%", "-50%"] }} transition={{ ease: "linear", duration: flowingReviews.length * 10, repeat: Infinity }} className="flex gap-10 items-stretch px-10 will-change-transform">
                    {flowingReviews.concat(flowingReviews).map((rev: any, i: number) => (
                        <div key={i} className="flex-shrink-0 w-[450px] bg-[#FAFAFA] border border-gray-100 p-10 rounded-[35px] flex flex-col justify-between group hover:border-[var(--theme-primary)]/40 transition-all duration-500 shadow-sm relative overflow-hidden">
                            {/* 🌟 GHOST REVIEW INDICATOR 🌟 */}
                            ...
                            <div>
                                ...
                                {/* 🌟 RENDER UPLOADED MEDIA IN REVIEW 🌟 */}
        ...
        <div className="max-w-4xl mx-auto px-6 md:px-10">
           ...
           <div className="space-y-4">
              {liveFaqs.length === 0 ? <p className="text-center text-gray-400 font-serif italic">No Questions added yet.</p> : liveFaqs.map((faq: any, i: number) => (
                <div key={faq._id || i} className={`bg-white rounded-[30px] overflow-hidden border transition-all duration-300 ${openFaq === i ? 'border-[var(--theme-primary)] shadow-md' : 'border-gray-100'}`}>
