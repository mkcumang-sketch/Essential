...
export default function PremiumAccountDashboard() {
    ...
    return (
        <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,rgba(212,175,55,0.18),transparent_62%),#050505] text-white font-sans selection:bg-[#D4AF37] selection:text-black pb-20 relative overflow-x-hidden">
            
            <AnimatePresence>
                {toastMsg && (
                    <motion.div initial={{ opacity: 0, y: 50, x: "-50%" }} animate={{ opacity: 1, y: 0, x: "-50%" }} exit={{ opacity: 0, scale: 0.9, x: "-50%" }} className="fixed bottom-10 left-1/2 z-[5000] bg-black/85 backdrop-blur-xl border border-[#D4AF37]/50 px-10 py-5 rounded-[28px] shadow-[0_20px_60px_rgba(212,175,55,0.18)] flex items-center gap-4">
            ...
            <header className="bg-black/70 backdrop-blur-xl border-b border-white/10 py-8 px-6 md:px-12 flex justify-between items-center sticky top-0 z-50 shadow-[0_18px_60px_rgba(0,0,0,0.45)]">
                <Link href="/" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[3px] text-white/70 hover:text-[#D4AF37] transition-colors">
                ...
                <div className="flex items-center gap-6">
                    {/* 🚨 NAYA LOGOUT BUTTON */}
                    <button onClick={handleLogout} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[3px] text-white/70 hover:text-black transition-colors px-6 py-3 rounded-full border border-white/10 hover:bg-[#D4AF37] hover:border-[#D4AF37] shadow-[0_20px_50px_rgba(212,175,55,0.08)]">
            ...
            <main className="max-w-[1600px] mx-auto pt-10 px-6 md:px-12 flex flex-col lg:flex-row gap-10 relative z-10">
                ...
                <aside className="w-full lg:w-[320px] shrink-0">
                    <div className="bg-white/5 backdrop-blur-xl border border-white/10 text-white p-10 rounded-[40px] mb-8 shadow-2xl relative overflow-hidden">
                        ...
                        <h3 className="font-serif text-3xl mb-1 relative z-10 text-white">{userName}</h3>
                    ...
                    <nav className="flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible custom-scrollbar pb-4 lg:pb-0 bg-white/5 border border-white/10 p-6 rounded-[40px]">
                        {MENU_ITEMS.map((item) => (
                            <button
                                key={item.id}
                                onClick={() => setActiveTab(item.id)}
                                className={`flex items-center gap-4 px-7 py-5 rounded-2xl text-[11px] font-black uppercase tracking-[4px] whitespace-nowrap transition-all w-full text-left border ${activeTab === item.id ? 'bg-[#D4AF37] text-black border-[#D4AF37] shadow-lg shadow-[#D4AF37]/20' : 'bg-transparent text-white/40 border-white/10 hover:bg-white/10 hover:text-white hover:border-[#D4AF37]/35'}`}
                            >
                                <item.icon size={18} className={activeTab === item.id ? 'text-black' : 'text-white/55'} /> {item.label}
                            </button>
                        ))}
                    </nav>
                </aside>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'OVERVIEW' && (
                            <motion.div key="1" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="p-10 bg-white/5 border border-white/10 rounded-[40px] flex flex-col justify-center hover:border-[#D4AF37]/35 transition-colors">
                                    ...
                                    <div className="p-10 bg-gradient-to-br from-[#1a1a1a] to-black border border-[#D4AF37]/30 rounded-[40px] flex flex-col justify-center relative overflow-hidden shadow-[0_25px_80px_rgba(212,175,55,0.10)]">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="bg-white/5 p-10 rounded-[46px] border border-white/10 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div onClick={() => showToast("Certificates are being verified. Check back later.")} className="bg-white/5 p-7 rounded-[34px] border border-white/10 flex flex-col items-center justify-center text-center hover:bg-white/10 cursor-pointer transition-all group hover:border-[#D4AF37]/35">
                                        ...
                                        <div onClick={() => showToast("Service Booking is currently offline.")} className="bg-white/5 p-7 rounded-[34px] border border-white/10 flex flex-col items-center justify-center text-center hover:bg-white/10 cursor-pointer transition-all group hover:border-[#D4AF37]/35">
                        ...
                        {activeTab === 'ORDERS' && (
                            <motion.div key="2" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="space-y-6">
                                {dashData?.orders?.length === 0 ? (
                                    <div className="bg-white/5 p-10 md:p-12 rounded-[46px] text-center border border-white/10">
                                    ...
                                    <div key={order._id} className="bg-white/5 p-10 rounded-[46px] border border-white/10 flex flex-col gap-6 hover:border-[#D4AF37]/50 transition-colors shadow-lg">
                        ...
                        {activeTab === 'EMPIRE_WALLET' && (
                            <motion.div key="3" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="space-y-6">
                                <div className="p-12 md:p-12 bg-gradient-to-br from-[#1a1a1a] to-black border border-[#D4AF37]/30 rounded-[46px] relative overflow-hidden shadow-2xl">
                                    ...
                                    <div className="mt-8 flex gap-4 relative z-10">
                                        ...
                                        <button onClick={() => showToast("No recent transactions.")} className="px-10 py-4 bg-white/5 border border-white/10 text-white text-[11px] font-black uppercase tracking-[5px] rounded-full hover:bg-white/10 hover:border-[#D4AF37]/35 transition-all">View History</button>
                                    </div>
                                </div>
                            </motion.div>
                        )}
                        ...
                        {activeTab === 'WISHLIST' && (
                            <motion.div key="4" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0}}>
                                <div className="bg-white/5 p-12 md:p-16 rounded-[46px] text-center border border-white/10">
                                    <Heart size={50} className="mx-auto text-gray-600 mb-6"/>
                                    <h3 className="text-2xl md:text-3xl font-serif text-white mb-2">Curated Collection</h3>
                                    ...
                                    <Link href="/shop" className="inline-block px-12 py-5 bg-white/10 text-white border border-white/20 text-[11px] font-black uppercase tracking-[5px] rounded-full hover:bg-[#D4AF37] hover:text-black transition-all shadow-[0_20px_60px_rgba(212,175,55,0.10)] border-[#D4AF37]/30">Explore Collection</Link>
                                </div>
                            </motion.div>
                        )}
                        ...
                        {activeTab === 'SETTINGS' && (
                            <motion.div key="5" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="space-y-6">
                                <div className="bg-white/5 p-10 rounded-[46px] border border-white/10">
                                    <h3 className="text-lg md:text-xl font-serif text-white mb-6 border-b border-white/10 pb-5 flex items-center gap-3"><MapPin size={20} className="text-[#D4AF37]"/> Saved Addresses</h3>
                                ...
                                <div className="bg-white/5 p-10 rounded-[46px] border border-white/10">
                                    <h3 className="text-lg md:text-xl font-serif text-white mb-6 border-b border-white/10 pb-5 flex items-center gap-3"><Shield size={20} className="text-[#D4AF37]"/> Account Security</h3>
                                    <div className="space-y-4">
                                        <p className="text-xs text-gray-400"><strong>Primary Email:</strong> {session?.user?.email}</p>
                                        <button onClick={() => showToast("Google Authenticated accounts cannot change passwords here.")} className="mt-4 w-full sm:w-auto flex items-center justify-center gap-4 text-[11px] uppercase tracking-[5px] font-black text-white/60 hover:text-[#D4AF37] transition-colors px-6 py-4 rounded-full border border-white/10 hover:border-[#D4AF37]/35 bg-white/5 hover:bg-white/10">
                                            Change Password
                                        </button>
                                    </div>
                                </div>
                        ...
                        {activeTab === 'SUPPORT' && (
                            <motion.div key="6" initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="space-y-6">
                                <div className="bg-white/5 p-12 md:p-16 rounded-[46px] text-center border border-white/10">
                                    ...
                                    <h3 className="text-3xl md:text-4xl font-serif text-white mb-4">Concierge Support</h3>
